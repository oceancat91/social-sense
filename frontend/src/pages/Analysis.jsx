...
function Analysis() {
  ...
  const sentimentBarOption = comparison.length > 0 && {
    title: { text: '各平台情感分布对比（情绪极化分析）', left: 'center' },
    ...
    series: ['positive', 'neutral', 'negative'].map(key => ({
      ...
      itemStyle: { color: SENTIMENTS[key].color },
      barMaxWidth: 45,
    })),
  }
  ...
  const propagationOption = propagation && propagation.items.length > 0 && {
    title: { text: '跨平台传播路径（舆情溯源）', left: 'center' },
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
        <h2 style={{ margin: 0 }}>跨平台舆情分析</h2>
        <div style={{ display: 'flex', gap: 12 }}>
          <Radio.Group value={days} onChange={e => setDays(e.target.value)} optionType="button">
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={12}>
          <Card>{sentimentBarOption && <ReactECharts option={sentimentBarOption} style={{ height: 360 }} />}</Card>
        </Col>
        <Col span={12}>
          <Card>{wordCloudOption && <ReactECharts option={wordCloudOption} style={{ height: 360 }} />}</Card>
        </Col>
      </Row>
